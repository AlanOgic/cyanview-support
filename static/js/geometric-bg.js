/**
 * Animated Geometric Background with Dynamic Word Formation
 * Canvas animation with floating shapes and configurable letter formations
 * Adapted for Cyanview Support site
 */
class GeometricBackground {
  constructor(canvasId, options = {}) {
    ...
    // Build letter array from word option
    const word = (options.word || 'CYANVIEW').toUpperCase();
    this.LETTERS = word.split('');
    const letterCount = this.LETTERS.length;

    // Auto-scale letter spacing based on word length
    const baseSpacing = 55;
    const autoSpacing = letterCount > 8
      ? Math.max(35, baseSpacing * (8 / letterCount))
      : baseSpacing;
    ...
    this.config = {
      ...
      rotationSpeed: options.rotationSpeed ?? { min: 0.001, max: 0.003 },
      sizeRange: { min: 15, max: 55 },
      letterSizeRange: { min: 35, max: 70 },
      baseOpacityRange: options.baseOpacityRange ?? null,
      letterMultiplier: options.letterMultiplier ?? 4,
      colors: {
        cyan: { r: 0, g: 168, b: 232 },
        white: { r: 255, g: 255, b: 255 },
      },
      mouseAttraction: { radius: 200, strength: 0.02, maxSpeed: 2 },
      formation: {
        ...
        letterSpacing: options.letterSpacing ?? autoSpacing,
        firstDelay: options.formationFirstDelay ?? 10000,
        yPosition: options.formationYPosition ?? 0.5,
      },
      background: options.background ?? { color1: '#122640', color2: '#091428' },
      drawBackground: options.drawBackground ?? true,
      drawConnections: options.drawConnections ?? true,
    };
    ...
    this.formation = { active: false, activeSet: -1, startTime: 0, phase: 'idle' };
  ...
  createLetter(char) {
    const letter = {
      ...
      baseOpacity: this.config.baseOpacityRange
        ? this.config.baseOpacityRange.min + Math.random() * (this.config.baseOpacityRange.max - this.config.baseOpacityRange.min)
  ...
  drawLetter(letter) {
    ...
    const color = boostLevel > 0.5 ? this.config.colors.cyan : (letter.isCyan ? this.config.colors.cyan : this.config.colors.white);
    this.ctx.save();
    ...
    this.ctx.font = `700 ${letter.size}px 'Phenomena', 'Outfit', 'Segoe UI', Arial, sans-serif`;
  ...
  getFormationTargets() {
    const totalWidth = (this.LETTERS.length - 1) * this.config.formation.letterSpacing;
    const startX = (this.canvas.width - totalWidth) / 2;
    const targetY = this.canvas.height * this.config.formation.yPosition;
    return this.LETTERS.map((_, i) => ({
  ...
  startFormation(time) {
    ...
    const letterCount = this.LETTERS.length;
    const setStart = this.formation.activeSet * letterCount;
  ...
  updateFormation(time) {
    ...
    if (this.formation.phase === 'forming') {
      ...
      for (let i = 0; i < letterCount; i++) {
        ...
        letter.formationOpacityBoost = 1 + easedProgress * 0.6;
      }
      if (progress >= 1) {
        ...
        for (let i = 0; i < letterCount; i++) {
          ...
          letter.rotation = 0;
          letter.formationOpacityBoost = 1.6;
        }
      }
      ...
      for (let i = 0; i < letterCount; i++) {
        ...
        letter.formationOpacityBoost = 1.6 - easedProgress * 0.6;
      }
      if (progress >= 1) {
  ...
  animate(time) {
    ...
    if (this.config.drawConnections) this.drawConnections();
